...
def test_trading_buying_exec() -> bool:
    ...
    try:
        with patch('trading.trading.AppConfigManager') as mock_app_config_manager_class, \
             ...
             # patch('broker.brokermanager.BrokerManager.initialize') as mock_bm_initialize, \
             # patch('broker.brokermanager.BrokerManager.finalize') as mock_bm_finalize:
        ...
        BrokerManager.finalize()
        mock_bm_finalize.assert_called_once()
...
def test_trading_selling_exec() -> bool:
    ...
    try:
        ...
        mock_bm_initialize.reset_mock()
        mock_bm_finalize.reset_mock()
        trading_instance.selling_exec([])
        mock_broker_instance.connect.assert_not_called()
        mock_broker_instance.disconnect.assert_not_called()
        mock_bm_initialize.assert_not_called()
        mock_bm_finalize.assert_not_called()
        dbg_info("Scenario 1: Passed")
        ...
        BrokerManager.initialize(broker_type='mock', simulation=True)
        mock_bm_initialize.assert_called_once()
...
def test_trading_selling_eval_flow() -> bool:
    ...
    try:
        with patch('trading.trading.BrokerManager') as mock_broker_manager_class, \
             patch('trading.trading.Evaluate') as mock_evaluate_class, \
             patch('broker.brokermanager.BrokerManager.initialize') as mock_bm_initialize, \
             patch('broker.brokermanager.BrokerManager.finalize') as mock_bm_finalize, \
            ...
            # Simulate BrokerManager being finalized externally
            BrokerManager.finalize()
            mock_bm_finalize.assert_called_once()
