...
class Position:
    ...
    def __init__(self, symbol: str, size: int = 0, average_entry_price: float = 0.0):
        self.symbol = symbol
        self.size = size
    ...
    def update(self, action: str, size: int, price: float):
        ...
        if action == 'buy':
            ...
            self.size -= size
            if self.size < 0:
                log.warning(f"Position size for {self.symbol} went negative ({self.size}). Resetting to 0.")
    ...
    def __repr__(self):
        return f"Position(symbol='{self.symbol}', size={self.size}, avg_entry={self.average_entry_price:.2f})"
...
class BaseBroker:
    ...
    def __init__(self, initial_cash: float = 1000000.0, commission_per_trade: float = 5.0):
        ...
        self.market = Market()
        log.info(f"Broker initialized with Cash: ${self.cash:,.2f}, Commission: ${self.commission_per_trade:.2f}/trade. State file: {self.state_filepath}")
    ...
    def place_order(self, symbol: str, action: str, size: int, price: float | None = None) -> dict | None:
        ...
        """
        if size <= 0:
            log.error(f"Order rejected for {symbol}: Size must be positive, got {size}.")
            return None
        if action not in ['buy', 'sell']:
            log.error(f"Order rejected for {symbol}: Invalid action '{action}'. Must be 'buy' or 'sell'.")
            return None

        # Get the current market price for execution comparison and potential fill
        market_price = self.get_last_price(symbol)
        if market_price <= 0.0:
             log.error(f"Order rejected for {symbol}: Invalid or zero market price ({market_price:.2f}) obtained.")
        ...
        if price is not None:
            if action == 'buy' and market_price > price:
                log.warning(f"Buy order for {symbol} rejected: Market price ({market_price:.2f}) > Limit price ({price:.2f})")
                return None
            elif action == 'sell' and market_price < price:
                log.warning(f"Sell order for {symbol} rejected: Market price ({market_price:.2f}) < Limit price ({price:.2f})")
                return None
            log.debug(f"Limit price condition met for {action} {symbol}: Market Price {market_price:.2f} vs Limit {price:.2f}")
        else:
            log.debug(f"Processing as market order for {action} {symbol} at Market Price {market_price:.2f}")
        ...
        log.debug(f"Attempting {action} order: {symbol}, Size: {size}, Execution Price: {market_price:.2f}, Cost/Proceeds: {cost:.2f}, Commission: {commission:.2f}")

        if action == 'buy':
            required_cash = cost + commission
            if self.cash < required_cash:
                log.warning(f"Order rejected for {symbol}: Insufficient cash. Required: ${required_cash:.2f}, Available: ${self.cash:.2f}")
                return None # Reject order

            # Update cash
            self.cash -= required_cash
            log.debug(f"Cash updated after BUY: ${self.cash:.2f}")

            # Update position
            if symbol not in self.positions:
                self.positions[symbol] = Position(symbol)
            self.positions[symbol].update(action, size, market_price) # Use market_price here
            log.info(f"Executed BUY: {symbol}, Size: {size}, Price: {market_price:.2f}. New Position: {self.positions[symbol]}")

        elif action == 'sell':
            current_position = self.get_position_by_symbol(symbol)
            if current_position.size < size:
                log.warning(f"Order rejected for {symbol}: Insufficient position to sell. Required: {size}, Available: {current_position.size}")
                return None # Reject order

            # Update cash
            proceeds = cost
            self.cash += (proceeds - commission)
            log.debug(f"Cash updated after SELL: ${self.cash:.2f}")

            # Update position
            self.positions[symbol].update(action, size, market_price) # Use market_price here
            log.info(f"Executed SELL: {symbol}, Size: {size}, Price: {market_price:.2f}. New Position: {self.positions[symbol]}")

            # Clean up position if size becomes zero
            if self.positions[symbol].size == 0:
                log.debug(f"Position closed for {symbol}. Removing from holdings.")
    ...
    def get_last_price(self, symbol: str) -> float:
        ...
        try:
            ...
            if result_df is not None and not result_df.empty:
                ...
                log.debug(f"Retrieved last price for {symbol}: {last_price}")
                return float(last_price) # Ensure it's a float
            else:
                log.warning(f"Could not get last price for {symbol}: No data returned or DataFrame is empty.")
                return 0.0 # Return 0.0 if no data is available
        except (KeyError, IndexError, TypeError) as e:
            log.error(f"Error retrieving last price for {symbol} from DataFrame: {e}")
            return 0.0 # Return 0.0 on error
        except Exception as e:
            log.error(f"An unexpected error occurred in get_last_price for {symbol}: {e}")
    ...
    def get_portfolio_value(self) -> float:
        ...
        log.debug(f"Portfolio Value Calculation: Cash ${self.cash:,.2f} + Holdings ${holdings_value:,.2f} = Total ${total_value:,.2f}")
        return total_value
    ...
    def save_state(self, filepath: str | None = None):
        ...
        try:
            ...
            log.info(f"Broker state saved successfully to {save_path}")
        except IOError as e:
            log.error(f"Failed to save broker state to {save_path}: {e}")
    ...
    def load_state(self, filepath: str | None = None):
        ...
        if not os.path.exists(load_path):
            log.warning(f"State file {load_path} not found. Initializing with default state.")
        ...
        try:
            ...
            log.info(f"Broker state loaded successfully from {load_path}. Cash: ${self.cash:,.2f}, Positions: {len(self.positions)}")

        except (IOError, json.JSONDecodeError, KeyError, TypeError) as e:
            log.error(f"Failed to load or parse broker state from {load_path}: {e}. Using existing/default state.")
            # Decide how to handle errors: keep current state, reset, or raise exception


if __name__ == "__main__":
    # --- Example Usage ---
    log.info("--- Starting Broker Example ---")
    ...
    for symbol, pos in all_positions.items():
        print(f"  {pos}")
    # Note: Portfolio value uses a dummy get_last_price
    print(f"Estimated Portfolio Value: ${broker1.get_portfolio_value():,.2f}")

    # 4. Save State
    print("\n--- Saving Broker State ---")
    broker1.save_state() # Saves to default "broker_state.json"

    # 5. Create a New Broker and Load State
    print("\n--- Creating New Broker (broker2) and Loading State ---")
    broker2 = BaseBroker(initial_cash=10000) # Start with different cash to show loading works
    print(f"broker2 Initial Cash: ${broker2.get_cash():,.2f}")
    broker2.load_state() # Loads from default "broker_state.json"
    ...
    loaded_positions = broker2.get_all_positions()
    if not loaded_positions:
        print("  No positions loaded.")
    for symbol, pos in loaded_positions.items():
        print(f"  {pos}")

    # 7. Demonstrate Custom Filepath
    print("\n--- Demonstrating Custom Filepath ---")
    custom_path = "data/custom_broker_state.json"
    broker1.set_state_filepath(custom_path)
    print(f"Set broker1 default path to: {broker1.state_filepath}")
    broker1.save_state() # Saves to the new default path

    broker3 = BaseBroker()
    print(f"\nCreating broker3. Initial Cash: ${broker3.get_cash():,.2f}")
    broker3.load_state(custom_path) # Load explicitly from the custom path
    print(f"broker3 Loaded Cash from {custom_path}: ${broker3.get_cash():,.2f}")
    print("broker3 Loaded Positions:")
    for symbol, pos in broker3.get_all_positions().items():
        print(f"  {pos}")

    # Clean up the created state files if desired
    # try:
    #     os.remove("broker_state.json")
    #     os.remove(custom_path)
    #     os.rmdir("data") # Remove directory only if empty
    #     print("\nCleaned up state files.")
    # except OSError as e:
    #     print(f"\nError cleaning up state files: {e}")

    log.info("--- Broker Example Finished ---")
