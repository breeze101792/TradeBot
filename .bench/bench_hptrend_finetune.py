"""
Quick final HPTrend tuning: only sweep slope_lookback and hp_lambda
with stop=12% and no TP (the best stop config).
"""
import sys
sys.path.insert(0, '.')
...
# Test grid: only slope_lookback and hp_lambda (fix stop=12%, tp=99%)
grid = []
for slope in [2, 3, 4, 5]:
    for lam in [3e4, 1e5, 3e5]:
        grid.append((f"sl{slope}-lam{int(lam/1000)}k", slope, lam))

print(f"HPTrend fine-tune: {len(grid)} configs x {len(sample)} stocks")
print(f"Fixed: stop=12%, TP=99% (no TP)")
print()
...
def run_backtest(df, slope, lam, name):
    cerebro = bt.Cerebro()
    ...
    cerebro.addstrategy(HPTrendV, slope_lookback=slope, hp_lambda=lam)
    cerebro.addanalyzer(bt.analyzers.AnnualReturn, _name='ar')
    results = cerebro.run()
    return results[0].analyzers.ar.get_analysis()


results = {g[0]: {} for g in grid}
import time
t0 = time.time()
for i, (gname, slope, lam) in enumerate(grid):
    for symbol, df in datas.items():
        ar = run_backtest(df, slope, lam, symbol)
...
for five_y, gname, yearly_avgs in ranked:
...
print(f"Best HPR-12-99 (from earlier run): 14.23%")
